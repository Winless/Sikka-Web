  ...
  <div>
    <div v-if="showdeposit">
      <a-modal v-model="visible" :footer="false" width="700px" wrapClassName="borrow" :afterClose="closemodel" :centered="true">
        ...
        <div class="progress">
          <div class="color1">{{ Number(depostit.schedule).toFixed(2) }}%</div>
          <div class="scale04">
            <div>25%</div>
            <div></div>
          </div>
    ...
    <EarnComplete v-if="showCompleteF" :completelist="this.completelist"></EarnComplete>
  </div>
</template>
...
import EarnComplete from "./Earn-Complete";


export default {
  name: "DepositF",
  components: { EarnComplete },
  props: {
    setShow: Function,
  },
  data() {
  ...
  methods: {
    ...
    async Balances() {
      ...
      this.depostit.Maxtoken = Number((+this.depostit.Maxtoken).toFixed(4))
