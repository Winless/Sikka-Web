...
export async function toAgreeProto(ths) {
    return new Promise((resolve, reject) => {
        let bf = (value) => {
            resolve(value)
        }
        ths.$bus.emit("showtermsConfirm", {value: true, callBack: bf})
    })
...
export function getScale() {
    ...
    if (screenWidth < 1280 && screenWidth > 750) {
        scale = (screenWidth - 96) / (1280 - 44);
    }
    return scale;
}
